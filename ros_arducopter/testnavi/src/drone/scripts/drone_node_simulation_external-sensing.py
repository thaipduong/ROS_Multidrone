...
class DroneDataAggregator:
    ...
    def __init__(self):
        ...
        print "Start GPS for drone is set! Start GPS is: (%f|%f|%f)" %(self.x, self.y, self.z)
...
class FakeDataModule:
    def __init__(self, xRangeDiameter, yRangeDiameter, zRangeHeight, resolutions):
        # Calculate explorable area grid bounds in terms of GPS locations based on input exploration area size
        self.GPSReceiver = DroneGPSSubscriber()
        self.GPSReceiver.setSubscribe()
        GPSLoc = self.GPSReceiver.getGPS()
        ...
        self.y = GPSLoc['longitude']
        self.z = GPSLoc['altitude']
        print "Start GPS for fake sensor data is set! Start GPS is: (%f|%f|%f)" %(self.x, self.y, self.z)
        xMax = self.x + (xRangeDiameter/2)*0.0003048/10000*90
        xMin = self.x - (xRangeDiameter/2)*0.0003048/10000*90
        yMax = self.y + (yRangeDiameter/2)*0.0003048/10000*90
        yMin = self.y - (yRangeDiameter/2)*0.0003048/10000*90
        ...
        self.sensePub = GasSensorPublisher()
...
def main():
    ...
    gasSensorModule = FakeDataModule(xDiam, yDiam, zDiam, resolutions)
