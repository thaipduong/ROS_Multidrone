...
class Drone:
    ...
    def __init__(self, xRangeDiameter, yRangeDiameter, zRangeHeight, resolutions, id):
        ...
        self.dataCollection = DroneDataAggregator()
        startGPS = self.dataCollection.GetGPS()
        ...
        self.dataCollection.InitFakeDataModule(bounds, resolutions)
...
class TestWaypointSubscriber:
    ...
    def WaypointReceive(self,msg):
        outstr = "Fake receiver received drone published waypoint: new objective at (%f|%f|%f)" %(msg.longitude, msg.latitude, msg.altitude)
        rospy.loginfo(outstr)
...
class TestGPSPublisher:
    ...
    def publishGPS(self, longi, lat, altitude):
        msg = GPS_Coord()
        msg.longitude = longi
        msg.latitude = lat
        msg.altitude = altitude
        outstr = "Fake GPS sending fake location at (%f|%f|%f)" %(msg.longitude, msg.latitude, msg.altitude)
        rospy.loginfo(outstr)
        self.pub.publish(msg)
...
class DroneGPSSubscriber:
    ...
    def GPSReceive(self,msg):
        ...
        rospy.loginfo(outstr)
    def getGPS(self):
        outstr = "Get location issued: location is now (%f|%f|%f)" %(self.longitude, self.latitude, self.altitude)
        rospy.loginfo(outstr)
...
class DroneWaypointPublisher:
    ...
    def setWaypoint(self, waypoint):
        ...
        outstr = "Set waypoint issued: objective is now (%f|%f|%f)" %(self.longitude, self.latitude, self.altitude)
        rospy.loginfo(outstr)
    def publishWaypoint(self):
        ...
        outstr = "Drone is publishing waypoint: objective is at (%f|%f|%f)" %(self.longitude, self.latitude, self.altitude)
        rospy.loginfo(outstr)
        self.pub.publish(msg)
...
class DroneDataAggregator:
    ...
    def InitFakeDataModule(self, bounds, resolutions)
        self.fakeData = FakeDataModule(self.x,self.y,self.z,self.xObjective, self.yObjective, self.zObjective, bounds, resolutions)
