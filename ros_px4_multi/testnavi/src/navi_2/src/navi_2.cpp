...
int main(int argc, char **argv)
{
  ...
  /*create ros node*/
  ros::init(argc, argv, "px4_navi_2");
  
  /*initialize handlers*/
  ros::NodeHandle nh;
  ros::Subscriber state_sub_ = nh.subscribe<mavros_msgs::State>("uav2/mavros/state", 10, state_callback);
  ros::Subscriber nextT_sub_ = nh.subscribe<sensor_msgs::NavSatFix>("FakeDroneObj_2", 10, nextT_callback);//geo msg
  ros::Subscriber gps_sub_ = nh.subscribe<sensor_msgs::NavSatFix>("uav2/mavros/global_position/global", 10, gps_callback);//geo msg
  ros::Subscriber signal_sub_ =nh.subscribe<std_msgs::Bool>("land_sig", 10, signal);

  ros::Publisher target_publisher_ = nh.advertise<mavros_msgs::GlobalPositionTarget>("uav2/mavros/setpoint_position/global", 10);
  ...
  ros::ServiceClient landing_client = nh.serviceClient<mavros_msgs::CommandTOL>("uav2/mavros/cmd/land") ;
  /*declare msg*/
