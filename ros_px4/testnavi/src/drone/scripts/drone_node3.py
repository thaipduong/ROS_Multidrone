...
class Drone:
    ...
    def UpdateLoc(self, measurement):
        ...
        if self.isExploring is True:
            print "Exploring: Drone %d moved to (%f|%f|%f), objective at (%f|%f|%f)" %(self.id, self.x, self.y,  self.z, self.xObjective, self.yObjective, self.zObjective)
        else:
            print  "Not exploring: Drone %d moved to (%f|%f|%f), objective at (%f|%f|%f)" %(self.id, self.x, self.y,  self.z, self.xObjective, self.yObjective, self.zObjective)
        ...
        if self.isExploring == True:
            if self.DistanceToObjective() < self.proximityReq:
                ...
                if numpy.any(currValMap > 0):
                    ...
                    print "Drone %d reached target: not exploring, moving to previously found event, new objective at (%f|%f|%f)" %(self.id, self.xObjective, self.yObjective, self.zObjective)
                    ...
                    print "Drone %d reached target: still exploring, moving to least explored, new objective at (%f|%f|%f), no event found yet" %(self.id, self.xObjective, self.yObjective, self.zObjective)
                    self.dataCollection.UpdateObjective(self.xObjective, self.yObjective, self.zObjective)
                time.sleep(5)
        else:
            if self.DistanceToObjective() < self.proximityReq:
                ...
                self.zObjective = leastExploredZ*self.zResolutionSize+self.bounds['zMin']
                self.isExploring = True
                print "Drone %d reached target: now exploring, moving to least explored, new objective at (%f|%f|%f), no event found yet" %(self.id, self.xObjective, self.yObjective, self.zObjective)
                self.dataCollection.UpdateObjective(self.xObjective, self.yObjective, self.zObjective)
                time.sleep(5)

class TestWaypointSubscriber:
    def __init__(self):
        rospy.Subscriber("droneObj", GPS_Coord, self.WaypointReceive)
        rospy.spin()
    def WaypointReceive(self,msg):
        print "Fake receiver detected drone published waypoint: new objective at (%f|%f|%f)" %(msg.longitude, msg.latitude, msg.height)
...
class TestGPSPublisher:
    def __init__(self):
        self.pub = rospy.Publisher('droneGPS', GPS_Coord, latch=True)
    def publishGPS(self, longi, lat, height):
        msg = GPS_Coord()
        msg.longitude = longi
        msg.latitude = lat
        msg.height = height
        print "Fake GPS sending fake location at (%f|%f|%f)" %(msg.longitude, msg.latitude, msg.height)
        self.pub.publish(msg)
...
class FakeDataModule:
    ...
    def FakeMove(self):
        ...
        if self.zObjective > self.z:
            ...
            self.z = self.z - 0.0003048/10000*90
        print "Fake move: location is now (%f|%f|%f)" %(self.x, self.y, self.z)
...
class DroneGPSSubscriber:
    def __init__(self):
        ...
        self.latitude = 1000
        self.height = 1000
        rospy.Subscriber("droneGPS", GPS_Coord, self.GPSReceive)
        rospy.spin()
    def GPSReceive(self,msg):
        self.longitude = msg.longitude
        self.latitude = msg.latitude
        self.height = msg.height
        print "Drone received GPS data: location is now (%f|%f|%f)" %(msg.longitude, msg.latitude, msg.height)
    def getGPS(self):
        print "Get location issued: location is now (%f|%f|%f)" %(self.longitude, self.latitude, self.height)
...
class DroneWaypointPublisher:
    def __init__(self):
        ...
        self.latitude = 1000
        self.height = 1000
        self.pub = rospy.Publisher('droneObj', GPS_Coord, latch=True)
    def setWaypoint(self, waypoint):
        self.longitude = waypoint['longitude']
        self.latitude = waypoint['latitude']
        self.height = waypoint['height']
        print "Set waypoint issued: objective is now (%f|%f|%f)" %(self.longitude, self.latitude, self.height)
    def publishWaypoint(self):
        msg = GPS_Coord()
        msg.longitude = self.longitude
        msg.latitude = self.latitude
        msg.height = self.height
        print "Drone is publishing waypoint: objective is at (%f|%f|%f)" %(self.longitude, self.latitude, self.height)
        self.pub.publish(msg)
...
class DroneSensorSubscriber:
    ...
    def SensorReceive(self,msg):
        ...
        print "Drone received sensor data: value is %f" %(self.value)
    def getValue(self):
